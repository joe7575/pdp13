--[[
	...
	PDP-13 Exam 5 (to get the Hard Disk tape)
...
local function exam_provide_number(pos, address, val1, val2)
	...
	mem.exam5_res = collatz(val)
	mem.exam5_time = minetest.get_gametime()
...
local function exam_check_result(pos, address, val1, val2)
	...
	mem.exam5_res = mem.exam5_res or 0
	mem.exam5_time = mem.exam5_time or 0

	if mem.exam5_time + 1 < minetest.get_gametime() then
		minetest.chat_send_player(owner, "[PDP13] Timeout reached!")
		return 0
	end
	
	local num = #mem.exam5_res
	local result = vm16.read_mem(pos, val1, num)
	
	for i = 1, num do
		if mem.exam5_res[i] ~= result[i] then
			pdp13.send_terminal_command(pos, mem, "println", "Expected value on index " .. i .. " is " .. mem.exam5_res[i])
...
|sys #| Exam5          | A    | B    | rtn  |
+-----+----------------+-------------+------+
 $308  request number    -      -     number
 $309  provide array    @result -     1=ok]]

pdp13.register_SystemHandler(0x0308, exam_provide_number, help)
pdp13.register_SystemHandler(0x0309, exam_check_result)
